#DESENVOLVEDOR: ADALBERTO DA SILVA ALMEIDA :)
...
print("A média de %s é %.2f" % (aluno,media)) # Imprime a média do aluno.
...
if media < 4.0: # Verifica se a média é menor que 4.
    ...
    if minMedia < 4.0: # Ajusta a média minima para a final, que n pode ser menor que 4.
        minMedia = 4.0
    print("%s deverá fazer a prova final e a média minima é %.2f." % (aluno, minMedia))
...
notaFinal = float(input("Digite a nota obtida na final:")) # Recebe a nota final.
mediaFinal = float((notaFinal + media) / 2) # Calcula a media final.
print("A media final foi : %.2f" %mediaFinal) # Imprime a média final.

'''Avaliação após a prova final'''

if mediaFinal >= 5.0: # Verifica se após a media final o aluno fou reprovado ou aprovado.
    print("%s foi APROVADO!" %aluno)
else:
    print("%s foi REPROVADO!" %aluno)
